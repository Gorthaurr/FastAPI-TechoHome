        ...
        try:
            ...
            if callback:
                try:
                    ...
                    logger.error(f"Error in error callback for image {image_id}: {callback_error}")
    ...
    async def reprocess_failed_images(self):
        ...
        try:
            failed_images = db.query(ProductImage).filter(
                ProductImage.status == "error"
            ).all()
            
            for image in failed_images:
                await self.process_image(image.id)
                
            logger.info(f"Added {len(failed_images)} failed images to reprocessing queue")
