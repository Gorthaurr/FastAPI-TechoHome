...
class SystemRunner:
    ...
    def __init__(self):
        ...
        self.uploads_dir = self.project_root.parent / "uploads"  # uploads в родительской папке
        self.cdn_cache_dir = self.project_root.parent / "cdn_cache"  # cdn_cache в родительской папке
        ...
        for docker_path in docker_paths:
            try:
                if os.path.exists(docker_path):
                    self.print_status(f"Запуск Docker Desktop: {docker_path}", Colors.YELLOW)
...
"""
                ...
                self.print_status("✅ .env файл создан с базовыми настройками", Colors.GREEN)
        else:
        ...
        try:
            ...
            try:
                result = subprocess.run([
                    'docker', 'ps', '--filter', 'ancestor=minio/minio', '--format', '{{.Names}}'
                ], capture_output=True, text=True, timeout=10)
                
                if result.returncode == 0 and result.stdout.strip():
                    container_name = result.stdout.strip()
                    self.print_status(f"Найден контейнер MinIO: {container_name}", Colors.GREEN)
                else:
                    self.print_status("⚠️  Контейнер MinIO не найден, пропускаем настройку bucket", Colors.YELLOW)
                    return True
                ...
                try:
                    ...
                    result = subprocess.run([
                        'docker', 'exec', container_name, 'mc', 'mb', 
                        f'local/{bucket_name}'
                    ], capture_output=True, text=True, timeout=10)
                    ...
                    if result.returncode == 0:
                        ...
                        self.print_status("✅ MinIO bucket уже существует", Colors.GREEN)
                    else:
                        self.print_status(f"⚠️  Ошибка создания bucket: {result.stderr}", Colors.YELLOW)
                    
                    # Устанавливаем политику public read
                    subprocess.run([
                        'docker', 'exec', container_name, 'mc', 'policy', 'set', 
                        'download', f'local/{bucket_name}'
                    ], capture_output=True, text=True, timeout=10)
                ...
                self.print_status(f"⚠️  Ошибка поиска контейнера MinIO: {e}", Colors.YELLOW)
                return False
    ...
    def wait_for_services(self):
        ...
        for service_name, host, port in services:
            ...
            for attempt in range(30):  # 30 попыток по 2 секунды = 1 минута
                ...
                time.sleep(2)
                if attempt % 7 == 0 and attempt > 0:  # Каждые 14 секунд показываем прогресс
                    self.print_status(f"Ожидание {service_name}... ({attempt + 1}/30)", Colors.YELLOW)
            else:
                self.print_status(f"❌ {service_name} не отвечает", Colors.RED)
        
        # Выводим итоговую статистику
        if ready_services:
            self.print_status(f"✅ Готово сервисов: {len(ready_services)}/{len(services)}", Colors.GREEN)
        ...
        if len(ready_services) < len(services):
            ...
            self.print_status("Система будет работать с ограниченной функциональностью", Colors.YELLOW)
    ...
    def start_fastapi(self):
        ...
        try:
            ...
            ], cwd=self.project_root.parent)  # Изменено: запускаем из родительской папки
    ...
    def print_system_info(self):
        ...
        print(f"{Colors.GREEN}🌐 FastAPI:     http://localhost:{self.fastapi_port}{Colors.END}")
        print(f"{Colors.GREEN}📚 API Docs:    http://localhost:{self.fastapi_port}/docs{Colors.END}")
        print(f"{Colors.GREEN}🔍 Health:      http://localhost:{self.fastapi_port}/healthz{Colors.END}")
        print(f"{Colors.GREEN}💾 MinIO API:   http://localhost:{self.minio_api_port}{Colors.END}")
        print(f"{Colors.GREEN}🖥️  MinIO Console: http://localhost:{self.minio_console_port}{Colors.END}")
        print(f"{Colors.GREEN}🗄️  PostgreSQL:  localhost:{self.postgres_port}{Colors.END}")
        print(f"{Colors.GREEN}🔴 Redis:       localhost:{self.redis_port}{Colors.END}")
        print(f"{Colors.GREEN}📊 pgAdmin:     http://localhost:{self.pgadmin_port}{Colors.END}")
        
        print(f"\n{Colors.BOLD}{Colors.YELLOW}🔑 Доступы:{Colors.END}")
        print(f"{Colors.YELLOW}MinIO - Логин: minioadmin, Пароль: minioadmin{Colors.END}")
        print(f"{Colors.YELLOW}pgAdmin - Email: admin@admin.com, Пароль: admin{Colors.END}")
        
        print(f"\n{Colors.BOLD}{Colors.PURPLE}📝 Полезные команды:{Colors.END}")
        print(f"{Colors.PURPLE}curl http://localhost:{self.fastapi_port}/healthz{Colors.END}")
        print(f"{Colors.PURPLE}curl http://localhost:{self.fastapi_port}/api/v1/cdn/stats/cache{Colors.END}")
        ...
        try:
            ...
            if docker_available:
                if not self.start_docker_services():
                    self.print_status("⚠️  Ошибка запуска Docker сервисов", Colors.YELLOW)
                    docker_available = False
