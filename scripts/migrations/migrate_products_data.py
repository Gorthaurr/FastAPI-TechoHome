...
class ProductDataMigrationService:
    ...
    def get_current_data_summary(self) -> Dict[str, Any]:
        ...
        result = self.db_session.execute(query)
        summary["sample_categories"] = [
            {"id": row.id, "slug": row.slug} for row in result
        ]
    ...
    def generate_data_report(self) -> None:
        ...
        if validation["orphaned_images"] > 0:
            report["recommendations"].append(
                f"Found {validation['orphaned_images']} orphaned product images"
            )

        if validation["products_without_images"] > 0:
            report["recommendations"].append(
                f"Found {validation['products_without_images']} products without images"
            )

        if validation["products_without_categories"] > 0:
            report["recommendations"].append(
                f"Found {validation['products_without_categories']} products without categories"
            )
        ...
        print(
            f"Products without Categories: {validation['products_without_categories']}"
        )
    ...
    def backup_current_data(self) -> str:
        ...
        # Backup products
        query = text(
            "SELECT id, name, description, price_raw, price_cents, category_id, product_url FROM products"
        )
        result = self.db_session.execute(query)
        ...
        result = self.db_session.execute(query)
        backup_data["categories"] = [{"id": row.id, "slug": row.slug} for row in result]
        ...
        backup_path = (
            ...
            / f'data_backup_{datetime.utcnow().strftime("%Y%m%d_%H%M%S")}.json'
        )
    ...
    def migrate_from_local_db(self) -> None:
        ...
        local_engine = create_engine(
            "postgresql+psycopg2://postgres:Anton533@localhost:5432/Shop"
        )
        ...
        try:
            ...
            local_categories = local_session.execute(
                text("SELECT id, slug FROM categories")
            ).fetchall()
            for category in local_categories:
                self.db_session.execute(
                    text(
                        "INSERT INTO categories (id, slug) VALUES (:id, :slug) ON CONFLICT (id) DO NOTHING"
                    ),
            ...
            local_products = local_session.execute(
                text(
                    """
            ...
            local_images = local_session.execute(
                text(
                    """
            ...
            local_attributes = local_session.execute(
                text(
                    """
