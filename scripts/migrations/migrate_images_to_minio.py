#!/usr/bin/env python3
"""
Migration script for moving product images from local storage to MinIO.
This script migrates all product images while preserving their database relationships.

Usage:
    python migrations/migrate_images_to_minio.py

Requirements:
    - MinIO server running on localhost:9002
    - PostgreSQL database with product_images table
    - Source images in C:\Users\demon\Desktop\парсинг\out
    - minio-py library installed
"""
...
class ImageMigrationService:
    ...
    def get_migration_data(self) -> List[Dict[str, Any]]:
        """Get all product image records from database."""
    ...
    def validate_source_files(self, migration_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Validate that source files exist on disk."""
        ...
        if missing_files:
            missing_file_path = Path(__file__).parent / 'missing_files.json'
            with open(missing_file_path, 'w', encoding='utf-8') as f:
                json.dump(missing_files, f, ensure_ascii=False, indent=2)
            logger.info(f"Missing files report saved to {missing_file_path}")
    ...
    def migrate_all_files(self, valid_records: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Migrate all valid files to MinIO."""
        ...
        for record in tqdm(valid_records, desc="Migrating to MinIO"):
            ...
            if result['migration_status'] == 'success':
                ...
                failed += 1
                logger.error(f"Migration error {record['filename']}: {result.get('error')}")
    ...
    def update_database(self, migration_results: List[Dict[str, Any]]) -> tuple:
        """Update database with new MinIO paths."""
    ...
    def generate_migration_report(self, migration_results: List[Dict[str, Any]]) -> None:
        """Generate detailed migration report."""
        logger.info("Generating migration report...")
        
        total = len(migration_results)
        successful = len([r for r in migration_results if r['migration_status'] == 'success'])
        ...
        report = {
            ...
            'failed_files': [
                ...
                for r in migration_results if r['migration_status'] == 'error'
            ]
        }
...
def main():
    print("=== PRODUCT IMAGES MIGRATION TO MINIO ===")
    print("This script will migrate all product images from local storage to MinIO.")
    print()
    print("Requirements:")
    print("1. MinIO server running on localhost:9002")
    print("2. PostgreSQL database with product_images table")
    print("3. Source images in C:\\Users\\demon\\Desktop\\парсинг\\out")
    print()
